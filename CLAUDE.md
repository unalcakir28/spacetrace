...
```bash
cargo test --workspace                   # 415 tests, all must pass
...
8. **Every long-run phase need counter that move.** Watcher answer "is it stuck" from counter alone — CLI warn after 10 second of still — so phase without one look hung while work. Clone probe do that: 1193 ms of 1989 ms scan on `~/github`, no counter move (measured 10 September 2026). Hence `clones_probed`. **That phase gone 22 September 2026** — clone family now arrive inside `getattrlistbulk` record, so `Phase::Finishing` be aggregation only and `clones_probed` stay 0 on macOS fast path. Counter keep, because slow path (directory hold mount point, platform with no bulk list) still ask one file at a time, and because it be on agent wire. **Save is phase too** (14 September 2026): 412,983 entry take 753 ms to walk, 571 ms to write, so ~14 second at 10M. `Phase::Saving`, `Phase::Checksumming` (273 ms write, 208 ms digest) and `ScanProgress::rows_done`/`rows_total` exist for this, CLI progress line cover save. **`StallWatch` read counter off `ScanProgress`**, so new counter reach every watcher unchanged.
...
- **btrfs and ZFS report real usage wrong**, because of reflink and compression. **APFS clone deduplicated**, on by default — this line say opposite until 11 September 2026 and contradict invariant 1 in same file. Mechanism change 22 September 2026: `ATTR_CMNEXT_CLONEID` + `ATTR_CMNEXT_EXT_FLAGS` inside bulk listing, key `(dev, clone_id)` gate on `EF_MAY_SHARE_BLOCKS`, charge in `place()` beside hardlink. Old way be `fcntl(F_LOG2PHYS_EXT)` probe per candidate in own phase after walk. Probe stay only for slow listing path (`bulk::clone_key`, `getattrlist` by path, no open). **64 KiB floor gone**, so small clone count too.
- **Scan hold whole tree in memory**: 106 byte/entry live (`Node` be 72 B plus name), linear from 100k to 10M. Peak above that platform-dependent and **still pile up on macOS**, because libmalloc no return fragmented span; not leak, agent unaffected. Measured 22 September 2026 on 1,064,452 entry: peak 299 → 200 MiB after walk stop build `PathBuf` per entry (only directory it descend into get one). Rest be 40% of peak and **no come back when tree drop**, so it be retention not tree. `expected_entries` hint make it *worse* there (191 vs 183 MiB), so arena doubling be not the cause. Number and argument: TODO.md D4. Tool: `cargo run --release -p spacetrace-scan-core --example memprobe -- scan <root>` and `scripts/bench-walk.sh`.
