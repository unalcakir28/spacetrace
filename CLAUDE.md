...
```bash
cargo test --workspace                   # 399 tests, all must pass
...
- **Agent have no built-in TLS**; put reverse proxy in front. **Rate limit exist** (`ratelimit.rs`): token bucket per client address, **before** auth, so it cover `/health` and wrong token — exactly the traffic that fall outside valid token. Behind proxy every request carry proxy address and limit collapse into one shared bucket; `X-Forwarded-For` deliberately not read.
